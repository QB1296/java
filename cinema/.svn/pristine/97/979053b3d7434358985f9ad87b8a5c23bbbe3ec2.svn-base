 ...
 */
public class SpiderJavassistLazyInitializer extends BasicLazyInitializer implements MethodHandler {

	private static final CoreMessageLogger LOG = Logger.getMessageLogger(CoreMessageLogger.class, SpiderJavassistLazyInitializer.class.getName());
	...
	private SpiderJavassistLazyInitializer(
	...
	public static HibernateProxy getProxy(
		...
		try {
			final SpiderJavassistLazyInitializer instance = new SpiderJavassistLazyInitializer(
					entityName,
					persistentClass,
					interfaces,
					id,
	...
	public Object invoke(
		...
		if ( this.constructed ) {
			...
			if ( result == INVOKE_IMPLEMENTATION ) {
				...
				if (thisMethod.getName().equals("isNullLazyProxy")) {
					if (target == null) {
						return true;
					}else {
						return false;
					...
					if (target == null) {
						throw new NoCriteriaWritingException("ToOne join 对象懒加载时需要先判断isNullLazyProxy方法");
					}
				}
